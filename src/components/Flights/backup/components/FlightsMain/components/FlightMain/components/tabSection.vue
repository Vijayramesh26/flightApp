<template>
    <div>
            ...
            <v-col>
                <v-tabs v-model="tab" background-color="transparent" color="TabTextColor" slider-color="SliderColor"
                    ...
                    <v-tab class="caption" :class="{ 'SliderColor--text font-weight-bold': tab === index }"
                        v-for="(tabs, index) in tabValues" :key="tabs" v-text="tabs"></v-tab>
                </v-tabs>
            </v-col>
            <v-col align="end">
        ...
        <FlightContent :currentDate="currentDate" />
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        generateWeeks() {
            ...
            this.ChoosedDate = days[0].date
            this.currentDate = days[0].dateKey
